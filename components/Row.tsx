import { IconButton, TableBody, TableCell, TableRow, Button, Chip, Collapse, Box, Table, TableHead, Grid, Typography, TableFooter } from '@material-ui/core'
...
function Row({ invoiceItem }: { invoiceItem: CustomInvoice }) {
  ...
  const patchData = async (url: string, data: dataType): Promise<CustomInvoice> => {
  ...
  const onSubmit: SubmitHandler<{ amount: number, id:number }> = async (data) => {
      ...
      <TableRow className={classes.root} hover>
        ...
        <TableCell align="left">{
          invoiceItem.status === 'BALANCE' ?
            ...
            /> :  invoiceItem.status === 'UNPAID' ? <Chip
      ...
      <TableRow>
        <TableCell style={{ paddingBottom: 0, paddingTop: 0 }} colSpan={6}>
          <Collapse in={open} timeout="auto" unmountOnExit>
            ...
            {
              invoiceItem.status === "PARTIAL_PAYMENT" || invoiceItem.status === "UNPAID" ?
                <Button variant="contained" color="primary" size="small" style={{ margin: 1 }}
                  onClick={() => triggerPayment(!payment)}>
                    Add Payment
                </Button> : 
                invoiceItem.status === "BALANCE" ? <Button variant="contained" color="primary" size="small"
                  style={{ margin: 1 }}
                  onClick={() => triggerPayment(!payment)}
                   >
                    Clear Balance
           ...
           
            {
              ...
              <Grid container spacing={1} direction="column">
                <Grid item xs={3}>
                  <Typography variant="subtitle1" style={{ fontWeight: 'bold' }} component="div">
